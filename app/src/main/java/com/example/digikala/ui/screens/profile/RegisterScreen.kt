...
fun RegisterScreen(
                ...
                is NetworkResult.Success -> {
                    loginResponse.data?.let { user ->
                        if (user.token.isNotEmpty()) {
                            ...
                            dataStore.saveUserPassword(viewModel.inputPasswordStat)
                            Constants.USER_PHONE = user.phone
                            Constants.USER_TOKEN = user.token
                            viewModel.screenStat = ProfileScreenStat.PROFILE_STAT

                        }
                    }
                    Toast.makeText(context, loginResponse.message, Toast.LENGTH_SHORT).show()
                    viewModel.loadingStat = false
                }

                is NetworkResult.Error -> {
                    viewModel.loadingStat = false
                    Log.e("3636", "login Error :${loginResponse.message}")
                }
    ...
    Column(
        ...
        Row(
            ...
            IconButton(onClick = { }) {
                Icon(
                    ...
                    tint = MaterialTheme.colors.selectBottom
                )
            }
        ...
        MyEditText(value = viewModel.inputPhoneStat, onValueChange = {
        }, placeholder = stringResource(id = R.string.phone_and_email))

        MyEditText(value = viewModel.inputPasswordStat, onValueChange = {
            viewModel.inputPasswordStat = it
        }, placeholder = stringResource(id = R.string.set_password))
        Spacer(modifier = Modifier.height(MaterialTheme.spacing.medium))
        if (viewModel.loadingStat) {
            LoadingButton()
        } else {
            MyButton(text = stringResource(id = R.string.digikala_login)) {
                if (isValidPassword(viewModel.inputPasswordStat)) {
                    viewModel.login()
                } else {
                    Log.e("3636", "login Error :${viewModel.inputPasswordStat}")
                    Toast.makeText(
                        context,
