    ...
    LazyColumn(
        ...
        item {
            MyEditText(value = viewModel.inputPhoneStat, onValueChange = {
                viewModel.inputPhoneStat = it
            }, placeholder = stringResource(id = R.string.phone_and_email))
        }
        item {
            MyButton(text = stringResource(id = R.string.digikala_entry)) {
                if (isValidEmail(viewModel.inputPhoneStat)
                    || isValidPhoneNumber(viewModel.inputPhoneStat)
                ) {
                    viewModel.screenStat = ProfileScreenStat.REGISTER_STAT
                } else {
                    Toast.makeText(
                        context,
